        ...
        <div className="relative">
          {!showSelectedBadge && (
            <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5">
              {isLoading ? <SearchLoadingIcon className="text-slate-400!" /> : <SearchInputIcon />}
            </div>
          )}
        ...
        <ul className="mx-1.5 mt-0.5 mb-1.5 list-none overflow-hidden rounded-[10px] border border-gray-200 bg-white p-0">
          {suggestions.map((item) => (
            <li key={`${item.type}-${item.value}`}>
              <button
                type="button"
                className="flex w-full cursor-pointer items-center justify-between gap-3 border-0 bg-[oklch(0.9851_0_0)] px-2.5 py-1.5 text-left font-[inherit] transition-all duration-75 hover:bg-[oklch(0.9214_0.0248_257.65)]"
